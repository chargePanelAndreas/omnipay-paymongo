...
class PurchaseRequest extends AuthorizeRequest
{
    ...
    public function getData()
    {
        ...
        $this->validate('amount', 'currency', 'paymentMethod', 'transactionId');

        $returnUrl = $this->getReturnUrl();

        // append query for the current paymentIntentId
        // we will retrieve this later on completePurchase and verify if
        // the off site payment (such as 3d secure) has been succeded
     ...
     */
    public function requestPaymentIntentId()
    {
        if ($id = $this->getPaymentIntentId()) {
            return $id;
        }

        // create a payment intent on the fly
        $request = new AuthorizeRequest($this->httpClient, $this->httpRequest);
        $response = $request->initialize($this->getParameters())->send();

        if (!$response->isSuccessful()) {
            throw new InvalidRequestException("Unable to create payment intent. Error: ". $response->getMessage());
        }

        $paymentIntentId = $response->getTransactionReference();
